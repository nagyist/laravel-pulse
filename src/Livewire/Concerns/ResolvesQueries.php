...
{
    /**
     * Resolve the query for the current period.
     *
     * @return array{0: mixed, 1: float, 2: string}
     */
    public function resolve(callable $query): array
    {
    ...
    public function remember(callable $query, string $key = ''): array
    {
        return Cache::remember(
            'laravel:pulse:'.static::class.':'.$key.':'.$this->period,
            $this->periodCacheDuration(),
            fn () => $this->resolve($query)
        );
    }
}
