                ...
                <div class="grid gap-3 mx-px mb-px">
                    @foreach ($queues as $queue => $readings)
                        <div wire:key="{{ $queue }}">
                            ...
                            @php
                                $highest = $readings->map(fn ($reading) => $reading->max())->max()
                            @endphp
                                    ...
                                    x-data="{
                                        init() {
                                                        ...
                                                        datasets: [
                                                            {
                                                                ...
                                                                data: @js($readings->get('queued')->map(fn ($value) => $value * (1 / $config['sample_rate']))),
                                                                order: 4,
                                                            },
                                                            {
                                                                label: 'Processing',
                                                                borderColor: 'rgba(147,51,234,0.5)',
                                                                data: @js($readings->get('processing')->map(fn ($value) => $value * (1 / $config['sample_rate']))),
                                                                order: 3,
                                                            },
                                                            {
                                                                label: 'Released',
                                                                borderColor: '#eab308',
                                                                data: @js($readings->get('released')->map(fn ($value) => $value * (1 / $config['sample_rate']))),
                                                                order: 2,
                                                            },
                                                            {
                                                                label: 'Processed',
                                                                borderColor: '#9333ea',
                                                                data: @js($readings->get('processed')->map(fn ($value) => $value * (1 / $config['sample_rate']))),
                                                                order: 1,
                                                            },
                                                            {
                                                                label: 'Failed',
                                                                borderColor: '#e11d48',
                                                                data: @js($readings->get('failed')->map(fn ($value) => $value * (1 / $config['sample_rate']))),
                                                                order: 0,
                                                            },
                                                        ],
                                                    },
                                            ...
                                            Livewire.on('queues-chart-update', ({ queues }) => {
                                                ...
                                                chart.data.labels = Object.keys(queues['{{ $queue }}'].queued)
